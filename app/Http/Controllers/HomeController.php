...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('home', compact('information', 'blog_exists', 'blogs', 'faq_exists', 'faqs', 'review_exists', 'reviews', 'monthly', 'yearly', 'prepaid', 'monthly_subscriptions', 'yearly_subscriptions', 'prepaids', 'languages', 'voices', 'max_chars', 'case_exists', 'cases'));
    }
    ...
    {
        $apiUrl = config('app.vaanee_api_endpoint') . 'api/check/';
        $client = new Client();
        $response = $client->get($apiUrl . '?username=' . $request->username);
        $responseBody = $response->getBody()->getContents();
        $responseData = json_decode($responseBody, true);
        if (is_array($responseData) && isset($responseData['msg']) && $responseData['msg'] == 'username already exists') {
            return 1;
        } else {
            return 0;
        }
