...
class VoiceCustomizationController extends Controller
{
    ...
    public function voices(Request $request)
    {
        ...
        return view('admin.studio.voices.index');
    }

       /**
     * List all voiceover voices
     */
    public function cloneVoices(Request $request)
    {
        if ($request->ajax()) {
            $data = Voice::select('voices.*', 'voiceover_languages.language', 'voiceover_languages.language_flag')->join('voiceover_languages', 'voices.language_code', '=', 'voiceover_languages.language_code')->where('voices.voice_type' , 'custome')->get();
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('actions', function($row){
                        $actionBtn = '<div>
    ...
    public function voiceUpdate(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($voice->vendor != 'aws') {
                ...
                return response()->json(["error" => __("AWS Voices cannot be renamed. It is a vendor limitation as AWS relies on initial voice names upon text synthesizing")], 422);
            }
        }
    }
