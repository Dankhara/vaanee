...
<div class="page-header mt-5-7">
	<div class="page-leftheader">
		<h4 class="page-title mb-0">{{ __('Record Transcribe Studio') }}</h4>
		<ol class="breadcrumb mb-2">
			<li class="breadcrumb-item"><a href="{{route('user.transcribe.record')}}"><i class="fa-solid fa-solid fa-headset mr-2 fs-12"></i>{{ __('User') }}</a></li>
			<li class="breadcrumb-item active" aria-current="page"><a href="{{url('#')}}"> {{ __('Record Transcribe Studio') }}</a></li>
		</ol>
	</div>
</div>
<!-- END PAGE HEADER -->
...
@section('content')
	<div class="row">
		<div class="col-lg-3 col-md-12 col-sm-12">
			<form id="transcribe-record" action="{{ route('user.transcribe.transcribe') }}" method="POST" enctype="multipart/form-data">
				@csrf
				...
				<div class="card border-0">
					<div class="card-body pt-6 pb-6">
						...
						<div id="record-actions" class="text-center">
							<!-- AUDIO FORMAT-->
							<div id="transcribe-audio-format">{{ __('Audio Format') }}: 1 channel PCM @ 48kHz</div>

							<!-- AUDIO RECORDER-->
							<div id="record-buttons">
								<div><button class="controls active" id="record"><i class="fa fa-microphone"></i>{{ __('Record') }}</button></div>
								<div><button class="controls is-blocked" id="stop" disabled="true"><i class="fa fa-stop-circle"></i>{{ __('Stop') }}</button></div>
							</div>
						...
						<div class="row">
							<div class="col-sm-12">
								<div class="form-group">
									<h6 class="task-heading">{{ __('Recording Language') }}</h6>
									<select id="languages" name="language" data-placeholder="{{ __('Select recording language') }}" data-callback="processLanguageFeature">
							...
							<div class="col-sm-12" id="removable-type">
								<h6 class="task-heading">{{ __('Recording Speaker Identification') }}</h6>
							...
							<div class="col-sm-12" id="removable-speaker">
								<div id="speakers-box">
									...
									<select id="speakers" name="speakers" data-placeholder="{{ __('Select max number of speakers in the recording') }}">
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('#resultTable').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('user.transcribe.record') }}",
				columns: [{
