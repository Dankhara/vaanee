...
<div class="page-header mt-5-7">
	<div class="page-leftheader">
		<h4 class="page-title mb-0">{{ __('Voiceover Projects') }}</h4>
		<ol class="breadcrumb mb-2">
			...
			<li class="breadcrumb-item active" aria-current="page"><a href="{{url('/' . $page='#')}}"> {{ __('Voiceover Projects') }}</a></li>
		</ol>
	</div>
</div>
<!-- END PAGE HEADER -->
...
@section('content')	
		...
		<div class="col-lg-12 col-md-12 col-sm-12">
			<div class="card border-0">	
				<div class="card-header">
					<h3 class="card-title">{{ __('All Voiceover Projects') }}</h3>
				</div>			
		...
		<div class="modal-dialog modal-dialog-centered modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header mb-1">
					<h4 class="modal-title" id="myModalLabel"><i class="fa-solid fa-rectangle-vertical-history"></i> {{ __('Select Default Project') }}</h4>
					<button type="button" class="btn-close fs-12" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<form action="{{ route('user.voiceover.project.update') }}" method="POST" enctype="multipart/form-data">
					@method('PUT')
					@csrf
					<div class="modal-body pb-0 pl-6 pr-6">        
						<div class="input-box">	
							<select id="set-project" name="project" data-placeholder="{{ __('Select Default Project') }}:">			
								@foreach ($projects as $project)
									<option value="{{ $project->name }}" @if (strtolower(auth()->user()->project) == strtolower($project->name)) selected @endif> {{ ucfirst($project->name) }}</option>
								@endforeach
							</select>
	...
	<div class="modal fade" id="deleteProjectModal" tabindex="-1" role="dialog" aria-labelledby="projectModalLabel" aria-hidden="true" data-bs-keyboard="false">
		<div class="modal-dialog modal-dialog-centered modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header mb-1">
					<h4 class="modal-title" id="myModalLabel"><i class="fa-solid fa-folder-xmark"></i> {{ __('Delete Project') }}</h4>
					<button type="button" class="btn-close fs-12" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<form action="{{ route('user.voiceover.project.delete') }}" method="POST" enctype="multipart/form-data">
					@method('DELETE')
					@csrf
					<div class="modal-body pb-0 pl-6 pr-6">       
						<p class="text-danger mb-3 fs-12">{{ __('Warning! This will also delete all synthesize results under selected project name') }}</p> 
						<div class="input-box">	
							<select id="del-project" name="project" data-placeholder="{{ __('Select Project to Delete') }}:">			
								@foreach ($projects as $project)
									<option value="{{ $project->name }}" @if (strtolower(auth()->user()->project) == strtolower($project->name)) selected @endif> {{ ucfirst($project->name) }}</option>
								@endforeach
							</select>
	...
	var table;
		$(function () {
			...
			table = $('#resultsTable').DataTable({
				...
				colReorder: true,		
				language: {
					"emptyTable": "<div><img id='no-results-img' src='{{ URL::asset('img/files/no-result.png') }}'><br>{{ __('Project does not have any results stored yet') }}</div>",
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('user.voiceover.projects') }}",
				columns: [{
		...
		function changeProjectName(value) {
			...
			$.get("{{ route('user.voiceover.projects.change') }}", { group: value}, 
				function(data){
					table = $('#resultsTable').DataTable({
						"lengthMenu": [[25, 50, 100, -1], [25, 50, 100, "All"]],
						responsive: {
			...
			$.get("{{ route('user.voiceover.projects.change.stats') }}", { project: value}, 
				function(data){
