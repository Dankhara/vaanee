...
<div class="page-header mt-5-7">
	<div class="page-leftheader">
		<h4 class="page-title mb-0">{{ __('Sound Mix Studio') }}</h4>
		<ol class="breadcrumb mb-2">
			...
			<li class="breadcrumb-item active" aria-current="page"><a href="{{ route('user.sound.studio') }}"> {{ __('Sound Mix Studio') }}</a></li>
		</ol>
	</div>
</div>
<!-- END PAGE HEADER -->
		...
		<div class="col-lg-12 col-md-12 col-sm-12">
			<div class="card border-0">
				<div class="card-header">
					<h3 class="card-title"><i class="fa-solid fa-photo-film-music mr-2 text-primary"></i> {{ __('Sound Mix Studio') }}</h3>
				</div>
							...
							<div class="row">
								...
								<div class="col-md-12 col-sm-12 text-center">
									<div class="dropdown">
										...
										<a class="btn btn-special create-project file-buttons pl-5 pr-5" href="{{ route('user.sound.studio.music.list') }}" id="list-music" data-tippy-content="{{ __('View All Your Uploaded Background Music Audio Files') }}">{{ __('View Music Files') }}</a>
									</div>
								</div>
							</div>
						</div>
		...
		<div class="col-lg-12 col-md-12 col-sm-12">
			<div class="card border-0">
				...
				<div class="card-body pt-2">
					<span class="text-muted fs-11">{{ __('Maximum rows to process is') }} {{ $row_limit }} <i class="ml-2 fa fa-info fs-8 info-notification" data-tippy-content="{{ __('Select rows that you want to merge together by clicking anywhere on the row. Click and drag with checkboxes to change the order of rows.') }}"></i></span>
					<!-- SET DATATABLE -->
	...
	<script type="text/javascript">
		$(function () {
			...
			let studio = $('#studioResultsTable').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('user.sound.studio.results') }}",
				columns: [
					{
						data: 'created-on',
						name: 'created-on',
				...
				$.each($("input:checked"), function(){
					...
					if (data !== undefined) {
						...
						if (user[11] == format) {
							...
							Swal.fire('{{ __('Incorrect Format Included') }}', '{{ __('Mixing audio file formats is not allowed, select exact audio formats as checked under <b>Audio File Format</b> section above.') }}', 'warning');
							process = false;
							return false;
						}
					}
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							method: 'post',
							url: 'sound-studio/result/delete',
							data: formData,
							processData: false,
							contentType: false,
							success: function (data) {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
							method: 'post',
							url: 'sound-studio/final/result/delete',
							data: formData,
							processData: false,
							contentType: false,
							success: function (data) {
								if (data == 'success') {
									Swal.fire('{{ __('Result Deleted') }}', '{{ __('Sound Studio result has been successfully deleted') }}', 'success');
									$("#studioResultsTable").DataTable().ajax.reload();
								} else {
									Swal.fire('{{ __('Delete Failed') }}', '{{ __('There was an error while deleting this sound studio result') }}', 'error');
								}
							},
							error: function(data) {
								Swal.fire({ type: 'error', title: 'Oops...', text: 'Something went wrong!' })
