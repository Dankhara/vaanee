...
@section('page-header')
    ...
    <div class="page-header mt-5-7">
        ...
        <div class="page-rightheader">
            <a href="{{ route('admin.settings.review.create') }}"
                class="btn btn-primary mt-1">{{ __('Create Customer Review') }}</a>
        </div>
    </div>
    <!-- END PAGE HEADER -->
@endsection
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $(document).on('click', '.deleteReviewButton', function(e) {
                ...
                Swal.fire({
                    title: '{{ __('Confirm Review Deletion') }}',
                    text: '{{ __('It will permanently delete this customer review') }}',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data == 'success') {
                                    Swal.fire('{{ __('Review Deleted') }}',
                                        '{{ __('Customer review has been successfully deleted') }}',
                                        'success');
                                    $("#faqsTable").DataTable().ajax.reload();
                                } else {
                                    Swal.fire('{{ __('Delete Failed') }}',
                                        '{{ __('There was an error while deleting this customer review') }}',
                                        'error');
                                }
                            },
                            error: function(data) {
