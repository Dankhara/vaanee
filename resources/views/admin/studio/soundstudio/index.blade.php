...
<div class="page-header mt-5-7">
	<div class="page-leftheader">
		...
		<ol class="breadcrumb mb-2">
			<li class="breadcrumb-item"><a href="{{url('/' . $page='#')}}"><i class="fa-solid fa-boxes-packing mr-2 fs-12"></i>{{ __('Admin') }}</a></li>
				...
				<div class="card-body pt-5">
					<div class="row">
						<div class="col-md-6 col-sm-12 w-100">
							<form class="w-100" action="{{ route('admin.sound.studio.store') }}" method="POST" enctype="multipart/form-data">
								@csrf
	...
	<script type="text/javascript">
		$(function () {
			...
			let list = $('#backgroundMusicTable').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('admin.sound.studio.music') }}",
				columns: [{
						data: 'created-on',
						name: 'created-on',
						orderable: true,
			...
			$(document).on('click', '.makePrivateButton', function(e) {
				...
				$.ajax({
					...
					method: 'post',
					url: 'sound-studio/music/private',
					data: formData,
					processData: false,
					contentType: false,
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
							method: 'post',
							url: 'sound-studio/music/result/delete',
							data: formData,
							processData: false,
							contentType: false,
							success: function (data) {
