...
@section('page-header')
    ...
    <div class="page-header mt-5-7">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">{{ __('Voices Customization') }}</h4>
                ...
                <div class="card-header">
                    ...
                    <ul class="nav nav-tabs">
                        <li class="active"><a data-toggle="tab" href="#home">
                                <h3 class="card-title">{{ __('All Voiceover Studio Voices') }}</h3>
                            </a>
                        </li>
                        <li><a data-toggle="tab" href="#tab1">
                                <h3 class="card-title">{{ __('Clone Voices') }}</h3>
                            </a></li>
                    </ul>

                </div>
                <div class="card-body pt-2 tab-content">
                    <!-- BOX CONTENT -->
                    <div class="box-content tab-pane fade in active" id="home">
                        <!-- SET DATATABLE -->
                    ...
                    <div class="box-content tab-pane fade" id="tab1">
                        <table id='cloneVoices' class='table' width='100%'>
                            <thead>
                                <tr>
                                    <th width="4%">{{ __('Status') }}</th>
                                    <th width="1%">{{ __('Avatar') }}</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header mb-1">
                    <h4 class="modal-title" id="myModalLabel"><i class="mdi mdi-animation"></i>
                        {{ __('Update Voice Name') }}</h4>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('#allResultsTable').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.voiceover.voices') }}",
                columns: [{
                        data: 'custom-status',
                        name: 'custom-status',
                        orderable: true,
            ...
            // INITILIZE DATATABLE
            var table = $('#cloneVoices').DataTable({
                "lengthMenu": [
                    [25, 50, 100, -1],
                    [25, 50, 100, "All"]
                ],
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.voiceover.clone.voices') }}",
                columns: [{
                        data: 'custom-status',
                        name: 'custom-status',
                        orderable: true,
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data == 'success') {
                                    Swal.fire('{{ __('Voice Name Updated') }}',
                                        '{{ __('Voice name has been successfully updated') }}',
                                        'success');
                                    $("#allResultsTable").DataTable().ajax.reload();
                                } else {
                                    Swal.fire('{{ __('Update Error') }}',
                                        '{{ __('Voice name was not updated correctly') }}',
                                        'error');
                                }
                            },
                            error: function(data) {
                                Swal.fire('Update Error', data.responseJSON['error'],
                                    'error');
                            }
                        })
                    } else if (result.dismiss !== Swal.DismissReason.cancel) {
                        Swal.fire('{{ __('No Voice Name Entered') }}',
                            '{{ __('Make sure to provide a name before updating') }}', 'error')
                    }
                })
            });
                ...
                Swal.fire({
                    ...
                    if (file.value) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data == 'success') {
                                    Swal.fire('{{ __('Avatar Uploaded') }}',
                                        '{{ __('Voice Avatar has been successfully uploaded') }}',
                                        'success');
                                    $("#allResultsTable").DataTable().ajax.reload();
                                } else {
                                    Swal.fire('{{ __('Upload Error') }}',
                                        '{{ __('Make sure you are uploading an image file') }}',
                                        'error');
                                }
                            },
                            error: function(data) {
                        ...
                        Swal.fire('{{ __('No File Selected') }}',
                            '{{ __('Make sure to select an image file before uploading') }}',
                            'error')
                    }
                })
            });
            ...
            $(document).on('click', '.activateVoiceButton', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 'success') {
                            Swal.fire('{{ __('Voice Activated') }}',
                                '{{ __('Voice has been activated successfully') }}',
                                'success');
                            $("#allResultsTable").DataTable().ajax.reload();
                        } else {
                            Swal.fire('{{ __('Voice Already Active') }}',
                                '{{ __('Selected voice is already activated') }}', 'error');
                        }
                    },
                    error: function(data) {
                        Swal.fire({
            ...
            $(document).on('click', '.deactivateVoiceButton', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 'success') {
                            Swal.fire('{{ __('Voice Deactivated') }}',
                                '{{ __('Voice has been deactivated successfully') }}',
                                'success');
                            $("#allResultsTable").DataTable().ajax.reload();
                        } else {
                            Swal.fire('{{ __('Voice Already Deactive') }}',
                                '{{ __('Selected voice is already deactivated') }}',
                                'error');
                        }
                    },
                    error: function(data) {
            ...
            $(document).on('click', '#activateAllVoices', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 'success') {
                            Swal.fire('{{ __('All Voices Activated') }}',
                                '{{ __('All voice were successfully activated') }}',
                                'success');
                            $("#allResultsTable").DataTable().ajax.reload();
                        }
                    },
            ...
            $(document).on('click', '#deactivateAllVoices', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 'success') {
                            Swal.fire('{{ __('All Voices Deactivated') }}',
                                '{{ __('All voice were successfully deactivated') }}',
                                'success');
                            $("#allResultsTable").DataTable().ajax.reload();
                        }
                    },
